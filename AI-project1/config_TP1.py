...
import robot_lara
import robot_dumb
import robot_sub_improved
import robot_sub_simple
import robot_sub_ultra_simple
import robot_sub_balanced
import robot_sub_FINAL
import robot_sub_OPTIMIZED
import robot_fin
...
def initialize_robots(arena_size=-1, particle_box=-1): # particle_box: size of the robot enclosed in a square
    ...
    robots = []
    for i in range(4):
        robots.append(robot_fin.Robot_player(x_init_pos[0], arena_size//2-16+i*8, orientation_champion, name="", team="Ny"))
    for i in range(4):
        robots.append(robot_champion.Robot_player(x_init_pos[1], arena_size//2-16+i*8, orientation_challenger, name="", team="PROF"))
    return robots
